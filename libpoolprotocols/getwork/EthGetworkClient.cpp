...
void EthGetworkClient::submitSolution(Solution solution)
{
	...
	if (m_connected || m_justConnected) {
		try
		{
			bool accepted = p_client->mib_submitWork("0x" + toHex(solution.nonce), "0x" + toString(solution.work.header), "0x" + toString(solution.mixHash));
			if (accepted) {
...
void EthGetworkClient::workLoop()
{
	while (true)
	{
		if (m_connected || m_justConnected) {
			...
			try
			{
				Json::Value v = p_client->mib_getWork();
			...
			if (m_submit_hashrate && !m_currentHashrateToSubmit.empty()) {
				try
				{
					p_client->mib_submitHashrate(m_currentHashrateToSubmit, "0x" + m_client_id.hex());
				}
				catch (const jsonrpc::JsonRpcException&)
				{
